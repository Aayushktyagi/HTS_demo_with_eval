...
if ($MKDAT) {
   ...
   if ($useDNN) {
      ...
      make_train_data_dnn();
      print "after making training data for deep neural network\n";
   }
}
...
if ($TRDNN) {
   ...
   if ($useDNN) {
      ...
      make_dnn_config();
      print "after make_dnn_config\n";
      shell("$PYTHON $datdir/scripts/DNNTraining.py -C $cfg{'tdn'} -S $scp{'fio'} -H $dnnmodels -z $var{'ffo'}");
   }
}
